...
public sealed class MailRelayHostnameTests
{
    ...
    public void Suggests_smtp_when_the_name_is_free()
    {
        ...
        Assert.Equal("smtp.aboutmyrental.com", suggestion.Hostname);
    ...
    public void Skips_to_relay_when_smtp_is_taken()
    {
        var suggestion = MailRelayProvisioningService.SuggestMailHostname(
            "aboutmyrental.com",
            [Record("smtp.aboutmyrental.com", "CNAME", "mail.protection.outlook.com")],
            "203.0.113.10");

        Assert.True(suggestion.Available);
        Assert.Equal("relay.aboutmyrental.com", suggestion.Hostname);
        Assert.Equal(["smtp.aboutmyrental.com"], suggestion.TakenHostnames);
    }
    ...
    public void Reuses_smtp_when_it_already_points_at_this_relay()
    {
        var suggestion = MailRelayProvisioningService.SuggestMailHostname(
            "aboutmyrental.com",
            [Record("smtp.aboutmyrental.com", "A", "203.0.113.10")],
            "203.0.113.10");
    ...
    public void Treats_proxied_or_foreign_a_records_as_taken()
    {
        var suggestion = MailRelayProvisioningService.SuggestMailHostname(
            "aboutmyrental.com",
            [
                Record("smtp.aboutmyrental.com", "A", "198.51.100.20", proxied: true),
                Record("relay.aboutmyrental.com", "A", "198.51.100.20"),
                Record("mail.aboutmyrental.com", "AAAA", "2001:db8::1")
            ],
            "203.0.113.10");
    ...
    {
        Assert.Equal(
            "relay.aboutmyrental.com",
            MailRelayProvisioningService.NormalizeMailHostname("relay", "aboutmyrental.com"));
    }
