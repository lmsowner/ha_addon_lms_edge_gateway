...
This Home Assistant App (formerly known as an Add-on) provides the install and lifecycle wrapper for LMS Edge Gateway.

Open the app through Home Assistant ingress after installation.
...
- `Verified LAN trust` (optional on MFA/Email approve IP routes): when Edge Gateway sees a real LAN source IP inside your trusted CIDRs and reverse/forward DNS matches your internal domain, authentication can be skipped for that request. This needs split/local DNS to the gateway. Cloudflare tunnel clients are never trusted this way.
...
- App repository URL for Home Assistant: https://github.com/lmsowner/ha_addon_lms_edge_gateway
