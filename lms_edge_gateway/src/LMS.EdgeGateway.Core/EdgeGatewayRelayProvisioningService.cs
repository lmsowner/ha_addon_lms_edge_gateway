...
public sealed class EdgeGatewayRelayProvisioningService(
    ...
    IWellKnownServiceStore? wellKnownServiceStore = null) : IEdgeGatewayRelayProvisioningService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var existing = File.Exists(configPath)
            ...
            : string.Empty;
        var candidatePath = $"{configPath}.candidate";
        ...
        await File.WriteAllTextAsync(candidatePath, caddyfile, cancellationToken);
        _ = await TryRunCommandAsync("caddy", ["fmt", "--overwrite", candidatePath], cancellationToken);

        var validate = await TryRunCommandAsync("caddy", ["validate", "--config", candidatePath, "--adapter", "caddyfile"], cancellationToken);
        if (!validate.Success && !IsCommandUnavailable(validate))
        {
            DeleteFileIfExists(candidatePath);
            return new CaddyApplyAttempt(
                false,
                $"Caddy validation failed; existing config was left unchanged: {FirstNonEmpty(validate.StandardError, validate.StandardOutput, validate.Message)}");
        }

        File.Move(candidatePath, configPath, overwrite: true);
        ...
        if (!string.IsNullOrWhiteSpace(existing))
        {
            ...
            _ = await TryRunCommandAsync("caddy", ["reload", "--config", configPath, "--adapter", "caddyfile"], cancellationToken);
        }
        else
        {
        ...
        var rollbackMessage = string.IsNullOrWhiteSpace(existing)
            ? "Caddy reload failed and the generated config was removed because there was no previous config to restore"
            : "Caddy reload failed and the previous config was restored";
        return new CaddyApplyAttempt(
            false,
            $"{rollbackMessage}: {FirstNonEmpty(reload.StandardError, reload.StandardOutput, reload.Message)}");
    }

    private string GenerateCaddyfile(EdgeGatewayConfiguration configuration)
        => GenerateCaddyfileWithWellKnown(configuration, []);

    private string GenerateCaddyfileWithWellKnown(
        EdgeGatewayConfiguration configuration,
        IReadOnlyList<WellKnownService> wellKnownServices)
    {
        ...
        WellKnownCaddyRouteRenderer.AppendRoutes(
            builder,
            wellKnownServices,
    ...
    private static bool IsCommandUnavailable(CommandAttempt attempt) =>
        attempt.Message.Contains("No such file or directory", StringComparison.OrdinalIgnoreCase) ||
        attempt.Message.Contains("No such file", StringComparison.OrdinalIgnoreCase) ||
        attempt.Message.Contains("not found", StringComparison.OrdinalIgnoreCase) ||
        attempt.Message.Contains("could not be started", StringComparison.OrdinalIgnoreCase);
